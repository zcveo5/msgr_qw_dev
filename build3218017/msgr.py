...
class Settings(ui.SidePanel):
    def __init__(self):
        ...
        self._base_widgets = [Button(self._o, text=locale['setting_sub_f_INTERFASE'],
                           ...
                           fg=default_fg, font=font_theme), Button(self._o, text=locale['setting_sub_f_FAQ'],
                           command=self.faq, bg=default_bg,
                           fg=default_fg, font=font_theme)
                              ]
    ...
    @staticmethod
    def faq():
        win = Popup(main, 450, 250, [600, 300], [default_bg, default_fg, font_theme],
                    title=locale['setting_sub_f_FAQ'])
        Button(win, text='Как и где хранятся мои данные?', command=lambda:
               show('Info', '1. Данные вашего аккаунта (Настройки, изменяемые в Настройках Аккаунта) хранятся на сервере.\n2. Ваши сообщения в приватных чатах хранятся локально и у вашего собеседника. Сообщения отправленые в General, сохраняются у всех кто прочитал ваше сообщение. Удалить сообщения у собеседника нельзя.')
    ...
    def sub_f_profile():
        ...
        def p_ip_check():
            ...
            try:
                ...
                if bt_server_data[1]['answer']['_show_ip'] == 'True':
                    ip_show_btn['text'] = locale['public_ip_unshow']
        ...
        if bt_server_data[1]['answer']['_show_ip'] == 'True':
            ...
            ip_show_btn = Button(window_prof, text=locale['public_ip_unshow'], command=p_ip_check, bg=default_bg, fg=default_fg, font=font_theme)
            ip_show_btn.pack(anchor='nw', padx=3)
    ...
    def sub_f_mod_rep():
        def get_mod_info(event):
            ...
            if mods_select.get(mods_select.curselection()) in ['core', 'backup', 'btac', 'Not connected to BebraTech connect']:
        ...
        #def load_repo_git():
        #    mods_select.delete("0", tk.END)
        #    url = f"https://api.github.com/repos/zcveo5-utility/plugin-repo/contents"
        #    headers = {
        #        "Accept": "application/vnd.github+json"}
        #    response = requests.get(url, headers=headers)
        #    show('EW', response.text)
        #    mods = []
        #    for _i in response.text:
        #        mods.append(_i['name'])
        #    mods_select.insert(tk.END, mods)
        #    mods_select.bind('<<ListboxSelect>>', lambda x: show('s', x))
        ...
        try:
            ...
            modlist = ['Not connected to BebraTech connect']
...
class Debug:
    ...
    def plug_create():
        ...
        def upload_plug():
            ...
            try:
                ...
                show('e', 'bebra tech connect not exists')
    ...
    def execute(code, use_exc_hook=False):
        ...
        if _handlers_dict['EXECUTE_MODE'] == 'NOT_THREAD': # if it fails you dont wrote execute mode: first str must be "#EXECUTE_MODE:THREAD/NOT_THREAD ...other handlers"
            exec__()
...
if os.path.basename(sys.argv[0]) in ['loader.py', 'launcher.pyw']:
    ...
    if bt_server == '' or server == '':
        pprint('[info] selecting connect')
        printin_load_lbl('Please, select connect')
    ...
    try:
        pprint('[info] connecting to account connect')
        ...
        if not bt_server_data[0] and bt_server_data[1]['answer'] != 'Incorrect password':
            ...
            if debug_mode:
                load_lbl['text'] += f'\nDebug Info:\ntd[0] -> false\n{bt_server_data}'
            action_load.configure(text='Reset BebraTech connect address', command=serv_sel_tmp)
            main.mainloop()
    ...
    try:
        pprint('[info] connecting to chat connect')
        ...
        if debug_mode:
            load_lbl[
                'text'] += f'\nDebug Info:\nIncorrect IP in <connect> variable.'
        action_load.configure(text='Reset Chatting connect address', command=serv_1_sel_tmp)
        main.mainloop()
